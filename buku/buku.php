...
<head>
    ...
    <meta name="keywords" content="masjid, baitul, maal, muslim, pkn, stan">
        <title>Buku dan Modul - MBM PKN STAN</title>
...
if (isset($_GET['pageno'])) {
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM tblposts_buku";
        $result = mysqli_query($con,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);


$query=mysqli_query($con,"select tblposts_buku.id as pid,tblposts_buku.PostTitle as posttitle,tblposts_buku.PostImage,tblcategory_buku.CategoryName as category,tblposts_buku.PostingDate as postingdate,tblposts_buku.PostUrl as url from tblposts_buku left join tblcategory_buku on tblcategory_buku.id=tblposts_buku.CategoryId where tblposts_buku.CategoryId='".$_SESSION['catid']."' and tblposts_buku.Is_Active=1 order by tblposts_buku.id desc LIMIT $offset, $no_of_records_per_page");

$rowcount=mysqli_num_rows($query);
if($rowcount==0)
{
echo "Tidak ditemukan buku atau modul dengan kategori ini.";
}
else {
             ...
             <div class="col-sm">
                ...
                <figure class="blog-item">
                    <figcaption>
                      ...
                      <a class="read-more" href="../file/<?php echo htmlentities($row['PostImage']);?>">Buka</a>
                    </figcaption>
                  </figure>
                </div>
